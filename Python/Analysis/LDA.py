...
class VariationalLDA(object):
    def load_data_from_h5ad(self, adata):
        expression_matrix = adata.X
        self.n_docs = expression_matrix.shape[0]
        self.word_index = {gene_name: idx for idx, gene_name in enumerate(adata.var_names)}
        self.n_words = len(self.word_index)
        self.corpus = {}
        for doc_idx in range(self.n_docs):
            self.corpus[f"cell_{doc_idx}"] = {gene_name: expression_matrix[doc_idx, gene_idx]
                                         for gene_idx, gene_name in enumerate(adata.var_names)
                                         if expression_matrix[doc_idx, gene_idx] > 0}
            self.make_doc_index()
    ...
    def _add_exact_fixed_topics(self,fixed_topics):
        ...
        self.beta_matrix = np.zeros((self.K,self.n_words),np.float32) + SMALL_NUMBER
        #self.beta_matrix[0:50,:] = fixed_topics
        for topic_pos,spectrum in fixed_topics:
                for word_pos,val in spectrum:
    ...
    def make_dictionary(self,metadata=None,min_prob_to_keep_beta = 1e-3,
        ...
        if metadata == None:
            if self.doc_metadata == None:
                metadata = {}
                for doc in self.corpus:
                    metadata[doc] = {'name': doc,'parentmass': (doc.split('_')[0])}
            else:
        ...
        return lda_dict
    def output(self):
        h5ad_file_path = "C:/Users/33652/Desktop/Data_Class/Skin.h5ad"
        adata = anndata.read_h5ad(h5ad_file_path)
        lda_model = VariationalLDA( K = 20, eta = 0.1, alpha = 0.1)
        lda_model.load_data_from_h5ad(adata=adata)
        df = adata.to_df()
        print(df.head())
        fixed_topics=[f"topic_{i}" for i in range(20)]
        lda_model.__init__()
        lda_model.create_word_index(df=df)
        lda_model.create_doc_index(df=df)
        lda_model.load_data_sc(df=df)
        lda_model.run_vb()
        lda_model.vb_step()
        init_alpha = np.random.uniform(low = 0.01, high = 1, size = 20)
        lda_model.alpha_nr()
        lda_model.e_step()
        lda_model.find_unique_words()
        lda_model.make_doc_index()
        lda_model.init_vb()
        for topic_id in range(20):
            result_doc_dict = lda_model.get_topic_as_doc_dict(topic_id)
            result_dict = lda_model.get_topic_as_dict( topic_id)
        doc_topic_distributions=lda_model.get_expect_theta()
        topic_word_distributions=lda_model.get_beta()

        # 将细胞（文档）对应的主题分布概率保存为CSV文件
        # 首先将doc_topic_distributions转换为DataFrame，行索引设为细胞编号（示例，可以根据实际情况调整更合理的索引）
        doc_topic_df = pd.DataFrame(doc_topic_distributions, index=[f"细胞_{i}" for i in range(len(doc_topic_distributions))])
        # 设置列名为主题编号
        doc_topic_df.columns = [f"主题_{i}" for i in range(1,21)]
        # 保存为CSV文件，这里将文件名设为'doc_topic_distributions.csv'，可根据需求修改
        doc_topic_df.to_csv("C:/Users/33652/Desktop/LDA_doc_topic_distributions.csv", index=True)
        
        # 将主题与基因（单词）之间的概率分布保存为CSV文件
        # 将topic_word_distributions转换为DataFrame，行索引设为主题编号
        topic_word_df = pd.DataFrame(topic_word_distributions,index=[f"topic_{i}" for i in range(1,21)])
        # 设置列名为基因名（从adata.var_names获取）
        topic_word_df.columns = adata.var_names
        # 保存为CSV文件，文件名设为'topic_word_distributions.csv'，可按需修改
        topic_word_df.to_csv("C:/Users/33652/Desktop/LDA_topic_gene_distributions.csv", index=True)
